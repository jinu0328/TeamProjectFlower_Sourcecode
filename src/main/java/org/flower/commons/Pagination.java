...
public class Pagination<T>{
    ...
    public Pagination(int _page, long _total, int _pagePerCnt, String url) {
        ...
        this.pageCnt = pageCnt < 1 ? 10 : pageCnt;          // 페이지 구간이 0 이하인 경우는 기본값  10 지정
        this.lastPage = (int)Math.ceil(total / (double)this.pagePerCnt);        // 마지막 페이지

        page = page < 1 ? 1 : page;                         // 페이지가 0 이하일 경우 1로 고정
        ...
        if(cnt == 0) this.isFirstCnt = true;            // 첫 번째 페이지 구간 체크
        ...
        for(int i = start; i < start + this.pageCnt; i++){
            ...
            if(i == this.lastPage) break;   // 마지막 페이지에서 멈춤
        }
        /* 구간별 페이지 번호 E */

        /* 전 구간 마지막 페이지 S */
        if(!this.isFirstCnt) prev = cnt * this.pageCnt;
        /* 전 구간 마지막 페이지 E */

        /* 다음 구간 시작 페이지 S */
        if(!this.isLastCnt) next = (cnt + 1) * this.pageCnt + 1;
        /* 다음 구간 시작 페이지 E */
    }
}
